...
from schemas.users import UserResponse, CreateUserRequest
...
def list_audit_events(actor_id: Optional[int] = None, action: Optional[str] = None, limit: int = 20, offset: int = 0):
    return {
        "total": 1,
        "items": [
            {
...
@router.get("/users/{user_id}/audit-events", tags=["audit-events"], response_model=AuditEventListResponse)
def get_user_audit_events(user_id: int, action:Optional[str] = None, limit: int = 20, offset: int = 0):
    return {
        "total": 1,
        "items": [
            {
